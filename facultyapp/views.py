...
def post_marks(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            from_email = 'kaustavrudra25@gmail.com'
...
def add_contact(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            form.save()
            return redirect('facultyapp:FacultyHomePage')  # Adjust to your contact list URL name
    else:
        form = ContactForm()
    return render(request, 'facultyapp/add_contact.html', {'form': form})
